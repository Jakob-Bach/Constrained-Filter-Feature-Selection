...
# Rename columns and also some values for the paper's plots. "long_metric_name" determines whether
# name + formal identifier or just the formal identifier are used for metrics. Note that the names
# used here are refered to in the evaluation scripts, so if you change something here, you should
...
# Rename columns and also some values for the dissertation's plots. "long_metric_name" determines
# whether name + formal identifier or just the formal identifier are used for metrics. Note that
# the names used here are refered to in the evaluation scripts, so if you change something here,
# you should also change it there.
def rename_for_diss_plots(results: pd.DataFrame, long_metric_names: bool = False) -> pd.DataFrame:
    results = results.copy()
    if 'split' in results.columns:
        results['split'] = results['split'].replace({'train': 'Train', 'test': 'Test'})
    if 'model' in results.columns:
        results['model'] = results['model'].replace(
            {'linear-regression': 'Lin', 'regression-tree': 'Tree',
             'xgb-linear': 'B-Lin', 'xgb-tree': 'B-Tree'}
        )
    results.rename(columns={'model': 'Prediction model', 'split': 'Split', 'r2': '$R^2$',
                            'constraint_name': 'Constraint type', 'dataset_name': 'Dataset',
                            'cardinality': 'Cardinality'}, inplace=True)
    if long_metric_names:
        results.rename(columns={
